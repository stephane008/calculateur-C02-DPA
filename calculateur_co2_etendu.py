...
def calculate_device_emissions(usage_hours, power_rating, num_devices):
    energy_used = usage_hours * power_rating * num_devices / 1000  # kWh
    co2_emissions = energy_used * 0.053  # Facteur d'émission moyen pour l'électricité
    return co2_emissions

st.title('Calculateur de CO2 Étendu')

st.header('Calcul des Émissions de CO2 pour les Déplacements et l\'Utilisation de Papier')
...
st.header('Calcul des Émissions de CO2 pour les Équipements Numériques')

usage_hours_devices = st.number_input('Heures d\'utilisation par jour (Équipements numériques)', value=0)
power_rating_devices = st.number_input('Puissance nominale de l\'équipement (Watts)', value=0)
num_devices = st.number_input('Nombre d\'équipements', value=0)

if st.button('Calculer les Émissions de CO2 pour les Équipements Numériques'):
    co2_devices = calculate_device_emissions(usage_hours_devices, power_rating_devices, num_devices)
    st.write(f'Émissions de CO2 pour les équipements numériques : {co2_devices:.2f} kg par jour')
